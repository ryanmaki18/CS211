...
This Python source file provides the classes Appt and Agenda.
"""
...
class Appt:
    ...
    def __lt__(self, other: 'Appt') -> bool:
        """Appointment a is before appointment b if finish time of a is equal or earlier than the start time of b. """
        return self.finish <= other.start

    def __gt__(self, other: 'Appt') -> bool:
        """Appointment a is after appointment b if the start time of a is equal or after the finish time of b."""
    ...
    def intersect(self, other: 'Appt') -> 'Appt':
        ...
        return Appt(max(self.start, other.start), min(self.finish, other.finish), self.desc + " & " + other.desc)
        ...
        for i in range(len(self.elements)):
            for j in range(i+1, len(self.elements)):
                if self.elements[i].overlaps(self.elements[j]):
                    conflict_list.append(self.elements[i].intersect(self.elements[j]))
                else:
                    break
