...
# The following operation codes control both the ALU and some other parts of the CPU.
...
class Instruction(object):
    ...
    def encode(self) -> int:
        ...
        encoded_int = 0
        encoded_int = op_field.insert(self.op.value, encoded_int)
        encoded_int = cond_field.insert(self.cond.value, encoded_int)
        encoded_int = reg_target_field.insert(self.reg_target, encoded_int)
        encoded_int = reg_src1_field.insert(self.reg_src1, encoded_int)
        encoded_int = reg_src2_field.insert(self.reg_src2, encoded_int)
        encoded_int = offset_field.insert(self.offset, encoded_int)
        return encoded_int
...
def decode(word: int) -> Instruction:
    ...
    op = OpCode(op_field.extract_signed(word))
    cond = CondFlag(cond_field.extract_signed(word))
    target = reg_target_field.extract_signed(word)
    src1 = reg_src1_field.extract_signed(word)
    src2 = reg_src2_field.extract_signed(word)
