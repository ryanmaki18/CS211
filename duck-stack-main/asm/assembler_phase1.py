...
class AsmSrcKind(Enum):
    ...
    # parts.
    MEMOP = auto()
    # Instruction that jumps to a location in memory and adds
    JUMP = auto()
...
ASM_MEMOP_PAT = re.compile(r"""
    ...
    \s+
    (?P<target>    r[0-9]+),            # Target register
    (?P<labelref> [a-zA-Z]\w*)          # src1, src2, offset all together
    \s*
...
# Instructions with a JUMP command. A pattern to recognize an instruction with just one operand, a memory address
